...
export default function Step2() {
  ...
  const [hpht, setHpht] = useState(d.hpht ?? '');
  const [show, setShow] = useState(false);
  const [children, setChildren] = useState(String(d.numberOfChildren ?? 0));
  const [medical, setMedical] = useState(d.medicalHistory ?? '');
  const [birth, setBirth] = useState(d.birthHistory ?? '');
  ...
  return (
    <Screen>
      <Title subtitle="Langkah 2 dari 3">Data Kehamilan</Title>
      <Text style={ui.label}>Hari pertama haid terakhir (HPHT)</Text>
      <Pressable style={ui.input} onPress={() => setShow(true)}>
        <Text>{hpht || 'Pilih tanggal'}</Text>
      </Pressable>
      {show && (
        <DateTimePicker
      ...
      <Field
        label="Hari perkiraan lahir (otomatis)"
        value={hpl}
        editable={false}
      />
      <Field
      ...
      />
      <Field
        label="Riwayat kesehatan (tulis ‘Tidak ada’ bila tidak ada)"
        value={medical}
