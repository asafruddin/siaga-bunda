...
Only the API performs transitions. Completing a posttest creates the next video's `pretest_required` row. A Vercel cron calls the protected reminder endpoint hourly; dashboard requests independently promote overdue schedules, so notification delivery is not a correctness dependency.
