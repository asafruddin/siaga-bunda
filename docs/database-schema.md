...
The API uses the service role and is the only writer. Anonymous database access is denied except for active video metadata. Researcher reporting uses `respondent_summary`, `video_monitoring`, `test_result_summary`, `test_comparison`, and researcher-only export views that may include direct respondent identifiers.
